...
4. إعدادات البناء تُكتشف تلقائياً (**Vite**): الأمر `npm run build`، المخرجات **`dist`**.
5. في Vercel → **Settings → Environment Variables** أضف على الأقل `VITE_ADMIN_PASSWORD` (وأي متغيرات من الجدول أعلاه).
6. **Custom Domain**: من المشروع في Vercel → **Domains** وأضف الدومين، ثم عدّل DNS عند مسجّل الدومين كما تطلبك Vercel.

ملف **`vercel.json`** يضيف إعادة توجيه SPA حتى تعمل مسارات مثل `/admin` و`/products` بعد التحديث المباشر.
